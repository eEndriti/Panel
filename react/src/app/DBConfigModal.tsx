...
const DBConfigModal: React.FC<DBConfigModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      callApi.loadDBConfig().then((cfg: DBConfig) => setConfig(cfg));
    }
  }, [isOpen]);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await callApi.saveDBConfig(config);
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg w-96 shadow-lg">
        ...
        <div className="mb-2">
          <label className="block font-medium">PDF Path:</label>
          <input
            type="text"
            className="border rounded w-full px-2 py-1"
            value={config.pdfSavePath}
            onChange={(e) => handleChange("pdfSavePath", e.target.value)}
          />
        </div>
